...
def train(hyp):
    ...
    #print(imgsz_min, imgsz_max, imgsz_test) 320,640,640
    # Image Sizes
    ...
    # Optimizer
    optimizer = torch.optim.SGD([model.patch], lr=1e-2, momentum=0.9,
                            weight_decay=0.0005)
    '''
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            #print(targets)
            #re-fine targets(classes)
            for i in range(targets.shape[0]):
                targets[i][1] = float(0)
                targets[i][2] = cfg_p.patch_x
                targets[i][3] = cfg_p.patch_y
                targets[i][4] = cfg_p.patch_w
                targets[i][5] = cfg_p.patch_h
            # Burn-in
            if ni <= n_burn:
            ...
            # Forward
            # pred (bsize, -1 , num_anchor, 11 (6+5))
            pred = model(imgs)
            #print(target)
            # Loss
        ...
        if save:
            print('-----------save %d patch to ------------'%epoch)
            save_patch(model.patch, epoch)
            #print(model.patch)
            print('\n')
